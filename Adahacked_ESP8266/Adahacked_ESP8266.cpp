...
// boolean Adafruit_ESP8266::postRequest(Fstr *url, char* json) {
boolean Adafruit_ESP8266::postRequest(Fstr *url, String json) {
    print(F("AT+CIPSEND="));
    println(58 + strlen_P((Pchr *)url) + json.length() + strlen_P((Pchr *)host));
    if(find(F("> "))) { // Wait for prompt
        print(F("POST ")); // 4
        print(url);
        print(F(" HTTP/1.1\r\nHost: ")); // 17
