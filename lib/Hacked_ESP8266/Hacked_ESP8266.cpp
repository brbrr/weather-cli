...
void Hacked_ESP8266::setBootMarker(Fstr *s) {
...
size_t Hacked_ESP8266::write(uint8_t c) {
  if(debug) {
    if(!writing) {
      debug->print(F("---> "));
      writing = true;
...
boolean Hacked_ESP8266::find(Fstr *str, boolean ipd) {
...
int Hacked_ESP8266::readLine(char *buf, int bufSiz) {
  if(debug && writing) {
    debug->print(F("<--- '"));
    writing = false;
  }
...
// false otherwise.
boolean Hacked_ESP8266::hardReset(void) {
  ...
  digitalWrite(reset_pin, HIGH); // Back to high-impedance pin state
...
boolean Hacked_ESP8266::softReset(void) {
  boolean  found = false;
...
void Hacked_ESP8266::debugLoop(void) {
...
boolean Hacked_ESP8266::connectToAP(Fstr *ssid, Fstr *pass) {
  // char buf[256];

  debug->print("AT+CWMODE=1");
...
void Hacked_ESP8266::closeAP(void) {
...
boolean Hacked_ESP8266::connectTCP(Fstr *h, int port) {
...
void Hacked_ESP8266::closeTCP(void) {
...
// need to parse IPD delimiters (see notes in find() function.
// (Can call find(F("Unlink"), true) to dump to debug.)
// boolean Hacked_ESP8266::requestURL(char* url) {
//     print(F("AT+CIPSEND="));
